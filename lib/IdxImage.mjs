import { idx2Hex } from './color256.mjs';
import ImgData from './ImgData.mjs';
...
export default class IdxImageData {
    ...
    toString(char) {
        const { realData: data, width, height, pixel } = this;
    ...
    getImgData() {
        const { realData, width, height } = this;
        const data = new Uint8ClampedArray(realData.length * 4);
        for(let i = 0, offset = 0; i < realData.length; i += 1, offset += 4) {
            const { r, g, b } = idx2Hex(realData[i]);
